...
class LookupRow(Scene):
    ...
    def construct(self):
        ...
        # ═══ Title ═══
        title = Text(
            "Look up every token in batch 0", font_size=28
        ...
        for t in range(T):
            ...
            num = Text(str(int(tokens[t])), font_size=16, color=WHITE).move_to(sq.get_center())
            input_cells.append(VGroup(sq, num))
        input_group = VGroup(*input_cells)
        input_caption = Text(
            "batch 0 tokens", font_size=13, color=BLUE_B
        ).next_to(input_group, LEFT, buff=0.2)
        ...
        for v, row in enumerate(emb_rows):
            lbl = Text(str(v), font_size=10, color=GREY_B).next_to(row, LEFT, buff=0.08)
            row_labels.add(lbl)
        table_label = Text(
            "embedding table", font_size=13, color=YELLOW_B
        ).next_to(table_group, UP, buff=0.15)
        ...
        out_label = Text("8 vectors", font_size=13, color=ORANGE).next_to(out_group, UP, buff=0.15)
        ...
        ).next_to(out_group, DOWN, buff=0.3)
