...
| 2b | User profile brief (job / learning types / materials / wants-as-quotes / personality / path kind) | Same call, same moment: `GET /plan?secret=...` prints the latest brief above the plan; `profile_brief_saved` in `/debug/timeline`. **Not gated** — the brief goes live for the coach's next send as soon as it is generated, so read it for wrong readings rather than to approve it. |
